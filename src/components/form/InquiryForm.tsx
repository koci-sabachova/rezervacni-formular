...
export function InquiryForm({ prefillPartySize }: { prefillPartySize?: number }) {
  ...
  const validationMsgs = useMemo<InquiryValidationMessages>(
    () => ({
      name: tVal("name"),
      ...
      partySizeMin: tVal("inquiryPartySizeMin"),
      partySizeMax: tVal("inquiryPartySizeMax"),
      eventType: tVal("inquiryEventType"),
      description: tVal("inquiryDescription"),
      gdpr: tVal("gdprInquiry"),
    }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [locale],
  );

  const schema = useMemo(() => createInquirySchema(validationMsgs), [validationMsgs]);
  ...
  const {
    ...
    defaultValues: {
      ...DEFAULT_VALUES,
      estimatedPartySize:
        prefillPartySize !== undefined ? prefillPartySize : DEFAULT_VALUES.estimatedPartySize,
    },
  ...
  async function onSubmit(values: InquiryInput) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        router.push(`/${locale}/thank-you?typ=inquiry`);
      } else {
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <label htmlFor="estimatedDate" className="field-label">
            ...
            <span className="text-[var(--color-text-subtle)] text-xs">({t("estimatedDateOptional")})</span>
          </label>
          <input id="estimatedDate" type="text" placeholder={t("estimatedDatePlaceholder")} className="input-base" {...register("estimatedDate")} />
          {errors.estimatedDate && <p className="field-error">{errors.estimatedDate.message as string}</p>}
        </div>
      </div>

      <div>
        <label htmlFor="eventType" className="field-label">
          {t("eventTypeLabel")} <span className="text-[var(--color-accent-soft)]">*</span>
        </label>
        <input id="eventType" type="text" placeholder={t("eventTypePlaceholder")} className="input-base" {...register("eventType")} />
        {errors.eventType && <p className="field-error">{errors.eventType.message as string}</p>}
      </div>

      <div>
        <label htmlFor="description" className="field-label">
          {t("descriptionLabel")} <span className="text-[var(--color-accent-soft)]">*</span>
        </label>
        <textarea id="description" rows={6} placeholder={t("descriptionPlaceholder")} className="input-base" {...register("description")} />
        {errors.description && <p className="field-error">{errors.description.message as string}</p>}
      </div>
      ...
      <TurnstileWidget
        onToken={(token) => setValue("turnstileToken", token, { shouldDirty: false, shouldValidate: false })}
      />
